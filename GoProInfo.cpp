...
typedef enum CameraEnum_t{NONE=0, \
    HERO4_Session, \
    HERO4_Black, \
    HERO4_Silver, \
    HERO3P_Black, \
...
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_LENS = AP4_ATOM_TYPE('L','E','N','S'); // GoPro Lens serial
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_CAME = AP4_ATOM_TYPE('C','A','M','E'); // GoPro Camera serial
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_SETT = AP4_ATOM_TYPE('S','E','T','T'); // GoPro Settings
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_AMBA = AP4_ATOM_TYPE('A','M','B','A'); // Ambarella defines
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_MUID = AP4_ATOM_TYPE('M','U','I','D'); // GoPro MUID
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_HMMT = AP4_ATOM_TYPE('H','M','M','T'); // GoPro Highlight tags
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_BCID = AP4_ATOM_TYPE('B','C','I','D'); // GoPro BCID

const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_CLIP = AP4_ATOM_TYPE('C','L','I','P'); // GoPro
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_PRNT = AP4_ATOM_TYPE('P','R','N','T'); // GoPro
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_GUMI = AP4_ATOM_TYPE('G','U','M','I'); // GoPro
const AP4_Atom::Type AP4_ATOM_TYPE_GPRO_INPT = AP4_ATOM_TYPE('I','N','P','T'); // GoPro
...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
static void
SetCameraType(AP4_Atom& atom)
{
    CameraEnum_t Camera = NONE;
    AP4_UI64 payload_size = atom.GetSize()-8;
    ...
    if (payload_size <= 1024) {
        ...
        payload->Seek(atom.GetHeaderSize());
       // if(payload->ReadString(FirmwareVersion, sizeof(FirmwareVersion))==AP4_SUCCESS)
        {
            payload->ReadString(FirmwareVersion, sizeof(FirmwareVersion));
            memcpy(CameraID,FirmwareVersion,6);
            
            CameraID[6] = '\0'; // shorten the string
...
static void
PrintHighLights(AP4_Atom& atom)
{
    AP4_UI64 payload_size = atom.GetSize()-8;
    unsigned int hightlight,num;
    ...
    if (payload_size <= 1024) {
        ...
        payload->Seek(atom.GetHeaderSize());
        payload->ReadUI32(num);
        
        if(num)
            printf("\t%d highlight%s\n", num,(num==1)?"":"s");

        do{
            payload->ReadUI32(hightlight);
            if(hightlight){
                printf("\t%d ms\n",hightlight);
             }
        }while(--num && hightlight != 0);
...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
static void
GetGoProSettings(AP4_Atom& atom)
{
    AP4_UI64 payload_size = atom.GetSize()-8;
    unsigned int sett1,sett2,sett3=-1;
    ...
    if (payload_size <= 1024) {
        ...
        payload->Seek(atom.GetHeaderSize());
        payload->ReadUI32(sett1);
        payload->ReadUI32(sett2);
        payload->ReadUI32(sett3);  // does not exist with GoPRO HERO3
...
main(int argc, char** argv)
{
    ...
    if (movie) {
        ...
        // Searching for GoPro Data Bits
        if(AP4_ContainerAtom* ilst = dynamic_cast<AP4_ContainerAtom*>(movie->GetMoovAtom()->FindChild("udta")))
        {
            int text;
            ...
            {
				text = -1; 
                AP4_Atom* uatom = static_cast<AP4_UnknownAtom*>(item->GetData());
                AP4_DataAtom* data_atom = static_cast<AP4_DataAtom*>(item->GetData());
                
                switch(AP4_Atom::Type type = uatom->GetType()){
                    case AP4_ATOM_TYPE_GPRO_FIRM:
                        SetCameraType(*data_atom);
                    break;
                        
                    case AP4_ATOM_TYPE_GPRO_LENS:
                        printf("\nLens Serial:\n\t");
                        text = 1;
                        break;
                        
                    case AP4_ATOM_TYPE_GPRO_CAME:
                        printf("\nGoPro Camera Serial (MD5 Encoded - HERO4):\n\t");
                        text = 0;
                        break;
                        
                    case AP4_ATOM_TYPE_GPRO_SETT:
                        printf("\nCamera Capture Settings:\n\t");
                        GetGoProSettings(*data_atom);
                        break;
                        
                    case AP4_ATOM_TYPE_GPRO_MUID:
                        printf("\nMedia Unique Identifier:\n\t");
                        text = 0;
                        break;
                        
                    case AP4_ATOM_TYPE_GPRO_HMMT:
                        printf("\nHightlight Tags:\n");
                        PrintHighLights(*data_atom);
                        break;
                ...
                if(text>=0){
                    ShowPayload(*data_atom, (text==1)?true:false);
